...
<script>
	...
	$(function () {
		$.ajax({
			method: "POST",
			url: "/Community/collectionCount.action",
			dataType: "json",
			success: function (arr) {
				for (var i = 0; i < arr.length; i++) {
					// 普通柱状图使用的数据
					nameArr.push(arr[i].collectionName+"/元");
					valueArr.push(arr[i].collectionMoney);
					// 南丁格尔玫瑰圆饼图使用的数据
					obArr.push({
						name: arr[i].collectionName+"/元",
						value: arr[i].collectionMoney
					});
				}
