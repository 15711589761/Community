	...
	layui.use('table', function () {
		...
		table.render({
			...
			, cols: [[
				...
				, {field: 'workPerson', width: 100, title: '维修员'}
				, {field: 'workTel', width: 120, title: '维修人员电话'}
		...
		$("#create_processes").on("click", function () {
			layer.open({
				type: 2,
				content: 'jsp/ljm_owner_dispatch.jsp',
				offset: ['10%', '40%'],
				title: '添加申请',
				area: ['300', '400px'],
				btn: ['添加申请', '关闭'],
				btn1: function (index, layero) {
					...
					if (name.length > 0) {
						if (phone.length > 0) {
							...
							$.ajax({
								url: "<%=path%>createProcessesForDispatch.action", //请求的url地址
								dataType: "json", //返回格式为json
								async: true,//请求是否异步，默认为异步，这也是ajax重要特性
								data: ob, //参数值
								type: "post", //请求方式
