...
<div class="layui-form">
	<div class="layui-form-item">
		...
		<div class="layui-inline">
			...
			<div class="layui-input-inline">
				<input type="time" class="layui-input" id="startTime" placeholder="HH:mm:ss">
			</div>
		</div>
		~
		<div class="layui-inline">
			...
			<div class="layui-input-inline">
				<input type="time" class="layui-input" id="endTime" placeholder="HH:mm:ss">
			</div>
		</div>
		<br>
		<div class="layui-inline" style="text-align: center; margin-left: 1000px">

			<input class="layui-input"  name="operation_person" id="operation_person" placeholder="请输入操作人" autocomplete="off">
		</div>
		<button class="layui-btn" data-type="reload">查询</button>
<%--		<button class="layui-btn" data-type="reload" lay-event="del">批量删除</button>--%>
		&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
		<button data-type="add" class="layui-btn layui-btn-normal">添加日志</button>
	...
	layui.use('table', function () {
		...
		table.render({
			...
			, cols: [[
				...
				,{field: 'operation_id',title: '编号', sort: true}
				, {field: 'operation_matter', title: '操作内容'}
				, {field: 'operation_person', title: '操作人', sort: true}
				, {field: 'operation_date', title: '操作日期'}
				, {field: 'operation_time', title: '操作时间'} //minWidth：局部定义当前单元格的最小宽度，layui 2.2.1 新增
		...
		var $ = layui.$, active = {
			reload: function () {
				var operation_person = $('#operation_person');
				...
				table.reload('testReload', {
					...
					, where: {
						operation_person: operation_person.val(),
		...
		table.on('tool(test)', function (obj) {
			...
			if (obj.event === 'detail') {
				...
				var operation_id = data['operation_id'];
				layer.confirm('真的删除行么', function (index) {
					$.ajax({
						type: "POST",//提交方式
						url: "/Community/deleMore.action?operation_id=" + operation_id,//提交的地址
						dataType: "json",//希望返回的数据类型
						async: true,//异步操作
				...
				layer.open({
					type: 2,
					title: '编辑',
					content: 'jsp/wsy_updateLog.jsp',
					maxmin: true,
					area: ['500px', '400px'],
					btn: ['修改', '取消'],
					success: function (layero, index) {
						var body = layer.getChildFrame('body', index);
						body.find("#operation_matter").val(data.operation_matter),
						body.find("#operation_person").val(data.operation_person)


					},
					yes: function (index, layero) {
						var operation_matter = $(layero).find('iframe')[0].contentWindow.operation_matter.value;
						var operation_person = $(layero).find('iframe')[0].contentWindow.operation_person.value;
						 var operation_id = data.operation_id;
						var ob = {
						...
						$.ajax({
							type: "POST",//提交方式
							url: "/Community/updateLog.action",//路径
							data: ob,//数据
							dataType: "json",//希望返回的数据类型
							async: true,//异步操作
