...
<script type="text/html" id="barDemo">
	<a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="agree">派工</a>
	<a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="disagree">不派工</a>
	...
	layui.use('table', function(){
		...
		table.on('tool(test)', function(obj){
			...
			} else if (obj.event === 'agree')
			{
				layer.open({
					type:2,
					content:"<%=path%>toDispatchPersonGet.view",
					offset: ['10%', '40%'],
					title: '派工',
					btn: ['派工', '关闭'],
					btn1: function (index,layero) {
						var workId = $(layero).find('iframe')[0].contentWindow.work_person.value;
						var agreeId = data.processTaskId
						var agreeOb = {taskId:agreeId,isAgree:"派工",workId:workId};
						$.ajax({
							url:"<%=path%>completeTaskWhitParameterForDispatch.action", //请求的url地址
							dataType:"json", //返回格式为json
							async:true,//请求是否异步，默认为异步，这也是ajax重要特性
							data:agreeOb, //参数值
							type:"post", //请求方式
			...
			} else if (obj.event === 'disagree')
			{
				...
				var disagreeOb = {taskId:disagreeId,isAgree:"不派工"};
				$.ajax({
