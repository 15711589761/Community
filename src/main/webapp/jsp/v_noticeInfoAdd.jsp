	...
	<script>
		...
		layui.use(['form', 'layedit', 'laydate'], function () {
			...
			form.on('submit(sure)', function (data) {
				...
				var theId=$("#staffId",parent.document).val();
				...
				layui.$.ajax({
					type: "post",
					url: <%=path%>+"addNewNotice.action",
					data: ob,
					dataType: "json",
					async: true,
					success: function (msg) {
						if (msg.identify==="addNewNotice") {
							if (msg.resultMsg=== "done"){
								var index = parent.layer.getFrameIndex(window.name); //先得到当前iframe层的索引
								parent.layer.close(index); //再执行关闭
								parent.layer.msg("新增成功！");//修改成功后提示！
								parent.layui.table.reload('noticeData');//刷新父界面的表格数据
							}
							else{
								layer.msg("新增失败，请重试！如果多次操作还是失败，请联系维护人员");
