...
<head>
  ...
  <link href=<%=path+"/ESMap/RouteAnalyser/css/common.css"%> rel="stylesheet">
</head>
<style type="text/css">
  #pannel {
    position: absolute;
...
<%--      </div>--%>
  ...
  <script src=<%=path+"/ESMap/lib/config.js"%>></script>
  <script src=<%=path+"/ESMap/lib/esmap.min.js"%>></script>
  <script src=<%=path+"/ESMap/lib/jquery-2.1.4.min.js"%>></script>
    <script src=<%=path+"/ESMap/lib/jquery.qrcode.min.js"%>></script>
    <script src=<%=path+"/ESMap/lib/tips_controls.js"%>></script>
  <script type="text/javascript">
    ...
    window.onload = function () {
      ...
      var ctlOpt = new esmap.ESControlOptions({
        ...
        imgURL: '<%=path+"/ESMap/RouteAnalyser/image/wedgets/"%>'
        // allLayer: true
        // size:"normal"
      })
      // 放大、缩小控件配置
      ...
      map = new esmap.ESMap({
        ...
        mapDataSrc: '<%=path+"/ESMap/data"%>', //地图数据位置
        mapThemeSrc:  '<%=path+"/ESMap/data/theme"%>', //主题数据位置
        ...
        if (coord != null) {
          ...
          if (clickCount == 0) {
            ...
            navi.setStartPoint({
              ...
              fnum: fnum,
              height: h,
              url: '<%=path+"/ESMap/RouteAnalyser/image/start.png"%>',
              size: 64
            });
            console.log(navi)
            ...
            navi.setEndPoint({
              ...
              fnum: fnum,
              height: h,
              url: '<%=path+"/ESMap/RouteAnalyser/image/end.png"%>',
              size: 64
            });
            // 画导航线
            navi.drawNaviLine();
    ...
    function createNavi() {
      ...
      if (!navi) {
        ...
        navi = new esmap.ESNavigation({
          map: map,
          locationMarkerUrl:'<%=path+"/ESMap/RouteAnalyser/image/pointer.png"%>',
